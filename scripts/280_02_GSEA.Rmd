...
We will construct separate networks for cancer (ER+) data from TCGA-BRCA and normal breast tissue data from GTEx. To achieve this goal, we outputed two expression datasets that can be used by PANDA analysis directly.
```{r}
...
Now, we will do a PANDA network analysis to calculate the difference in transcriptions factors regulations.
...
```
Since running PANDA takes a long time, we ran it on an online platform and downloaded the results
...
We conducted PANDA analysis to generate two networks for tumor (ER+) samples and normal samples by command lines. 
...
The large number of edges and nodes (317) result in a network with substantial overlapping. We construct separate PANDA networks for gained and lost regulation by selecting the strongest positive and negative differential TF–gene interactions between ER+ breast tumors and normal breast tissue.
...
write.table(panda_neg, file = "../output/panda_ERpos_tumor_vs_normal_topneg_edges.txt",
            sep = "\t", row.names = FALSE, quote = FALSE)
```
`panda_pos` captures TF–gene regulatory edges with increased strength in tumor relative to normal, whereas `panda_neg` captures edges that are weakened in tumor and stronger in normal.
